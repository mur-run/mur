...
- Pages are **plain Markdown** in `dashboard/docs-content/`, no front-matter (start with `# Title`). They render at `/docs/core/<slug>` — the slug is the filename without `.md`, loaded by `getDocBySlug` (`dashboard/src/app/docs/core/[[...slug]]/page.tsx`). Adding a file makes the route exist; **it is only linked once you add a nav entry.**
- **Navigation** — add `{ title: '<Title>', href: '/docs/core/<slug>' }` to the right group in `dashboard/src/components/docs/coreNavigation.tsx` (groups: Getting Started / Features / Integrations / Resources).
