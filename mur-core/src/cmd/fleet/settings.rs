//! `mur fleet set-loop` mutate fleet's `loop:` block (trigger, budget,
...
/// Mutate fleet's `loop:` block. Only fields `Some(..)` are
/// changed; everything else in existing `loop_cfg` (or `FleetLoop`'s own
/// per-field defaults if fleet has no `loop_cfg` yet) is preserved.
/// Merge semantics matter: partial update (e.g. "just bump budget")
/// must never silently reset trigger/deadline/done_when someone already set.
...
mod tests {
	...
	fn set_loop_merges_onto_existing_config_without_clobbering_other_fields() {
		...
		assert_eq!(l.deadline, "", "untouched field uses FleetLoop's own default");
		assert_eq!(l.done_when, "", "untouched field uses FleetLoop's own default");
	}
}
