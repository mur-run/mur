...
pub fn assemble_file(
    ...
    for base_unit in sorted_units {
        let Some(selection) = plan.selections.get(&base_unit.name) else {
            continue;
        };
        ...
        let Some(replacement) = track_units.iter().find(|u| u.name == base_unit.name) else {
            continue;
        };
...
mod tests {
    ...
    fn replaces_winning_unit() {
        ...
        assert!(
            result_str.contains("hi there!"),
            "expected replacement: {result_str}"
        );
        assert!(
            result_str.contains("farewell"),
            "expected farewell preserved: {result_str}"
        );
    }
}
