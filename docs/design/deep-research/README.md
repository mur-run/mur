...
**Advisory-enforcement honesty.** The egress proxy now, before opening a CONNECT tunnel, resolves the target host and screens the resolved IP — dropping link-local (`169.254.*`, `fe80::`), unspecified (`0.0.0.0`, `::`), and cloud-metadata IPs with `403` — then connects to the pinned `SocketAddr` to close the DNS-rebinding window. This hardens **traffic routed through the proxy** against SSRF metadata-endpoint and rebinding attacks. However, a `Restricted` worker's OS policy still allows direct egress to `*:80/443/8080/8443` (from `from_entitlements`), so a compromised subprocess can bypass the proxy entirely. Full airtight egress (forcing all traffic through the proxy via a loopback-only SBPL mode) is a deliberate follow-up. Tier 1 (`reqwest` HTTP) and tier 2/3 (browser subprocesses via obscura's `--proxy` or native lightpanda) both go through the proxy audit; obscura is opt-in and proxy-governed, and lightpanda is native proxy-governed and faster. The next phase-3 sbpl pin-to-proxy would seal the remaining direct-connect gap.
