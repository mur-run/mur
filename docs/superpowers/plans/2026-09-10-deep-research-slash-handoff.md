...
- [x] **Step 3: docs trio.** `README.md` deep-research section: add `/deep-research` and the `wait:false` + `mur_job_status` flow. Docs site and product page sources are **not in this repo** (`mur-server/dashboard/docs-content/` does not exist here; `CLAUDE.md:158` points at https://app.mur.run/docs/core and /products/mur) — the follow-up was filed as #1324 and shipped in mur-server#89 (new `/docs/core/deep-research` page, registered slug, Features nav entry, product-page card). That PR also corrected the README: `fleet_run`'s `wait` parameter no longer exists — the tool is unconditionally detached now — so the docs describe the current handle-always behaviour, not `wait:false`.
