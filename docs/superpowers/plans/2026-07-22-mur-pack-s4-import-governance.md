...
> **Revisions (during/post execution, per revised spec §4.1 + final review):** (1) `AddonRef` also gained `fetch_plugin: Option<String>` (the `--plugin` marketplace selector), captured at import and threaded back through `reimport` — without it, reimport of a marketplace add-on (e.g. superpowers) was a guaranteed destroy-then-fail. (2) `reimport` is **atomic-on-failure**: the previous add-on's on-disk dirs are backed up before removal and restored if EITHER the remove step or the re-import fails; the backup path is surfaced in the error. (3) The pre-sync race (import before the first `mur sync` of a release misses a not-yet-synced builtin) is documented-ACCEPTED — `mur skill doctor` (S1) detects the shadow post-hoc and `reimport` corrects it; `list_installed(...).unwrap_or_default()` fail-open is intentional per that contract. (4) Two workspace-EXCLUDED `mur-hub-gui` `AddonRef` test literals needed the three new fields (fixed in PR #745 — see `mem:gotcha_workspace_excluded_addonref_literals`).
