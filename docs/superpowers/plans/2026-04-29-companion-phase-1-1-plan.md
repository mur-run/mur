...
- [ ] **Step 2: Run test, verify FAIL** (`field 'companion' missing` or similar)
...
- [ ] **Step 3: Run test, verify PASS** (`cargo test -p mur-common --test schema_evolution`)
...
- [ ] **Step 2: Run test, verify FAIL** (`anyhow!("M1.5 stub")`)

- [ ] **Step 3: Implement non-interactive `init`**
...
- [ ] **Step 2: Implement** — current `init.rs` already supports `re_init` via the `if !re_init &&` check; ensure ledger / bandit-state / inbox **are not touched**. Add explicit comments and a test exercising it.
...
- [ ] **Step 1: Author 10 templates** following Spec Appendix A.1/A.2 format (use `{{NAME_FOR_USER}}`, `{{FORMALITY}}`, `{{EXTRA_INSTRUCTIONS}}`, `{{LOCALE}}` placeholders only).

- [ ] **Step 2: Write `voice_template.rs`**
...
- [ ] **Step 1: Write Clock + Voice tests**
...
- [ ] **Step 3: Implement voice composer**
...
- [ ] **Step 2: Implement `compose_with_overrides`** that checks per-agent dir → user dir (`~/.mur/companion/templates/`) → embedded.
...
- [ ] **Step 1: Test** (`mur-agent-runtime/tests/durable_ledger.rs`)
...
- [ ] **Step 2: Implement** — header parsing per Spec §6.3, 529 multiplier 4×–8× (use 6× as midpoint), full-jitter formula `min(cap, base * 2^attempt) * rand_uniform`.
...
- [ ] **Step 2: Implement** — `StubLlm` parses `stub_scenarios.yaml`; `generate` hashes the request and returns the first matching scenario, or default echo. Selection by `MUR_LLM_PROVIDER=stub` in `llm/mod.rs::client_from_env`.
...
- [ ] Author seed yamls; ensure each has ≥3 templates per situation per locale.
- [ ] Implement `TemplateState` struct (per Spec §3.4) + `BanditState { morning_sent_today, templates: BTreeMap<TemplateId, TemplateState> }`.
- [ ] Test: round-trip serde, seed loader produces non-empty pool per situation.
- [ ] Commit: `M4.1: content pool seed (per Spec Appendix B) + bandit-state types`.
...
- [ ] Test: empty pool → None; single eligible → returns it; equal weights → 1:1 ±5% over 200 picks (seeded RNG); weight cap 5.0; weight floor 0.1; cooldown filter excludes recent.
- [ ] Implement `Picker::pick`, `Picker::record` exactly per Spec §4.5.
- [ ] Commit: `M4.2: picker WeightedIndex + cooldown + record(Signal)`.
...
- [ ] Test: 06:30 returns morning_greeting eligible; 12:00 morning ineligible; 23:00 returns None (or all zero).
- [ ] Implement `pick_for_hour(now_local, morning_sent_today)`.
- [ ] Commit: `M4.3: situations weight table + morning_greeting once-per-day cap (Spec §4.6)`.
...
- [ ] Test: 12h active window, daily_cap=3, no prior send, now=09:00 → desired_interval=4h, should_send=true (since elapsed=∞); after sending, advance 1h → false; advance 4h → true; advance into quiet_hours → false.
- [ ] Implement per Spec §4.7. ActiveHours/QuietHours parsing (HH:MM with chrono-tz aware).
- [ ] Commit: `M4.4: schedule::should_send_now (deterministic interval + jitter; Spec §4.7)`.
...
- [ ] Test: proactive disabled → blocked; paused_until in future → blocked; learning_until in future → blocked; quiet_hours match → blocked.
- [ ] Implement `pub fn check(profile: &CompanionConfig, now_utc, now_local) -> GateOutcome`.
- [ ] Commit: `M4.5: earned_permission gate (proactive/paused/learning/quiet)`.
...
- [ ] Implement `OutboxEvent` enum exactly per Spec §3.5 (12 variants including `RhythmWiped`).
- [ ] Test: every variant serde-roundtrips.
- [ ] Commit: `M5.1: OutboxEvent enum (12 variants; frozen schema, Spec §3.5)`.
...
- [ ] Test: `StdoutNotifier::send` writes `inbox/<ulid>.md` with front-matter + body + `>>> response: <unset>`; second call with same id refuses (`O_CREAT|O_EXCL`).
- [ ] Implement Notifier trait per Spec §4.9; inbox markdown format per Spec §3.6.
- [ ] Commit: `M5.2: Notifier trait + StdoutNotifier + inbox markdown writer (R16 O_EXCL)`.
...
- [ ] Test (uses StubLlm + FakeNotifier + MockClock): proactive disabled → 24h sim 0 sends; enabled cap=3 + 12h window → exactly 3 sends.
- [ ] Implement steps 1-7 (gates + scheduling) per Spec §4.8. Steps 8-12 land in M5.4-M5.6.
- [ ] Commit: `M5.3: outbox tick scheduling core (gates → schedule → ledger MessageScheduled)`.
...
- [ ] Test linter: banned phrases, ≤1 emoji, ≤1 exclamation, length 1-3 sentences, zh-TW preserved-English ratio ≤30%.
- [ ] Test outbox: linter pass → MessageGenerated + MessageSent; first fail → regenerate; second fail → MessageDropped { reason: "linter_persistent" }.
- [ ] Implement linter + integrate (per Spec §4.8 step 9 and Spec §8.5 C1).
- [ ] Commit: `M5.4: linter heuristic + regenerate-once integration in outbox`.
...
- [ ] Test: stub forces English when locale=zh-TW → translate path runs → MessageSent; translate stub returns 429 → MessagePaused { resume_at }; advance MockClock past resume_at → next tick resumes; 4 retries fail → MessageDropped { reason: "locale_unresolved" }.
- [ ] Implement `i18n::ensure_locale` per Spec §6.2 + outbox steps 10-12 + resume step 2.
- [ ] Commit: `M5.5: outbox i18n + rate-limit pause/resume + retry queue (Spec §4.8 + §6.2)`.
...
- [ ] Test: send message at T; advance MockClock 24h+1m; tick → PassiveDismissInferred event + picker.dismiss_count incremented.
- [ ] Implement step 3 of `run_tick` (Spec §4.8).
- [ ] Commit: `M5.6: outbox passive-dismiss sweep (sent>24h + no signal → infer dismiss)`.
...
- [ ] Implement `Companion::new(profile, clock) -> Option<Self>` returning `None` when `!profile.companion.enabled`.
- [ ] Add 60s tick from supervisor calling `companion.run_tick(now)` when `Some`.
- [ ] Test: agent without companion block → no companion module instantiated (memory ≤200B, Spec Q9).
- [ ] Commit: `M5.7: wire Companion into supervisor tick (zero-cost when disabled, Q9)`.
...
- [ ] Refuse if `companion/inbox/*.md` non-empty unless `--force`.
- [ ] Test: unread message → remove fails with hint; --force succeeds.
- [ ] Commit: `M6.10: agent remove refuses with unread companion inbox (R13)`.
...
- [ ] M1.1: workspace deps
- [ ] M1.2: companion enums
- [ ] M1.3: CompanionConfig on AgentProfile
- [ ] M1.4: schema-evolution guard
- [ ] M1.5: CLI scaffold
- [ ] M1.6: init --answers (atomic + flock)
- [ ] M1.7: interactive wizard
- [ ] M1.8: re-init preserves history

**M2 — Voice + i18n**
- [ ] M2.1: embed template matrix + locale fallback
- [ ] M2.2: Clock + voice composer
- [ ] M2.3: disk override lookup chain
- [ ] M2.4: i18n heuristic_matches

**M3 — Durable + Stub**
- [ ] M3.1: durable::Ledger
- [ ] M3.2: durable::rate_limit
- [ ] M3.3: StubLlm provider

**M4 — Picker + Situations + Schedule**
- [ ] M4.1: content seed + bandit-state types
- [ ] M4.2: picker WeightedIndex + cooldown
- [ ] M4.3: situations weight table + morning cap
- [ ] M4.4: schedule deterministic interval
- [ ] M4.5: earned_permission gates

**M5 — Outbox + Notifier**
- [ ] M5.1: OutboxEvent enum
- [ ] M5.2: Notifier + StdoutNotifier + Inbox
- [ ] M5.3: outbox tick core
- [ ] M5.4: linter + regenerate-once
- [ ] M5.5: i18n + rate-limit pause/resume
- [ ] M5.6: passive-dismiss sweep
- [ ] M5.7: supervisor wire

**M6 — CLI**
- [ ] M6.1: proactive enable/disable
- [ ] M6.2: quiet
- [ ] M6.3: voice eject/rebuild/diff
- [ ] M6.4: templates eject
- [ ] M6.5: content add
- [ ] M6.6: inbox + ack
- [ ] M6.7: preview
- [ ] M6.8: why-did-you-message
- [ ] M6.9: rhythm wipe
- [ ] M6.10: agent remove unread guard

**M7 — Unit + Snapshot**
- [ ] M7.1: insta fixtures
- [ ] M7.2: picker math + scenario
- [ ] M7.3: ledger fixtures + reflective coverage
- [ ] M7.4: telemetry no-PII sentinel
- [ ] M7.5: voice quality C1
- [ ] M7.6: cargo bench smoke

**M8 — Integration + E2E**
- [ ] M8.1: harness helpers
- [ ] M8.2: gating integration tests (5)
- [ ] M8.3: rate-limit + i18n integration tests (4)
- [ ] M8.4: signal/restart integration tests (7)
- [ ] M8.5: E2E companion-phase11.sh
- [ ] M8.6: Ollama nightly smoke

**M9 — Docs**
- [ ] M9.1: README quickstart
- [ ] M9.2: CLAUDE.md
- [ ] M9.3: CONTRIBUTING.md
- [ ] M9.4: spec post-impl notes
- [ ] M9.5: PR + retro
