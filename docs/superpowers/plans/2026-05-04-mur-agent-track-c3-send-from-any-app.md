...
- [ ] **Step 1: Failing test** — `mur-agent-gui/src-tauri/src/send/mod.rs` with inline test module.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test send::tests` (the module doesn't exist yet, so `lib.rs` must also `pub mod send;`).
- [ ] **Step 3: Implement** — add `pub mod send;` to `mur-agent-gui/src-tauri/src/lib.rs`. Empty stubs for `url_scheme`/`hotkey`/`services`/`dock`.
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test send::tests`.
- [ ] **Step 5: Commit** — `M-c3.0.1: SharePayload + ShareKind round-trip`.

### Task M-c3.0.2: `SendIngestor` trait + default implementation

**Files:** Modify `mur-agent-gui/src-tauri/src/send/mod.rs`

- [ ] **Step 1: Failing test** — append to `send/mod.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_ingest_routing`. `mur_agent_runtime::ledger::append_share_entry` does not yet exist.
- [ ] **Step 3: Implement** — add `append_share_entry(agent_home, source, content)` to `mur-agent-runtime/src/ledger.rs`: writes `{"ts": ..., "source": ..., "content": ...}` line into `<agent_home>/telemetry/inputs/share.jsonl`. Wire `mime_guess` and `async_trait` deps in `mur-agent-gui/src-tauri/Cargo.toml`. (`// tauri-2 / objc2-0.5 surface — verify on impl`)
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_ingest_routing`.
- [ ] **Step 5: Commit** — `M-c3.0.2: DefaultIngestor routes Text/Url to share.jsonl, Image/File to D3 pipeline`.
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test b0_share_wrapping`.
- [ ] **Step 3: Implement** — extend `on_prompt_submit`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test b0_share_wrapping` and confirm pre-existing `b0_pdf_wrapping` / `b0_tool_result_wrapping` tests still pass.
- [ ] **Step 5: Commit** — `M-c3.0.3: B0SafetyHook recognizes <untrusted_share> + sets cooldown`.

### Task M-c3.0.4: One-turn tool-cooldown end-to-end

**Files:** Modify `mur-agent-runtime/tests/b0_share_wrapping.rs`

- [ ] **Step 1: Failing test** — share content followed by a tool call must trigger Rule 4 AskUser:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test b0_share_wrapping`.
- [ ] **Step 3: Implement** — share content already sets `after_untrusted_input` from M-c3.0.3. The Rule 4 same-turn check in `B0SafetyHook::on_pre_tool_use` already inspects this flag (M3.8). The only new wiring is the `test_env::deliver_share` helper that round-trips through `on_prompt_submit`.
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test b0_share_wrapping`.
- [ ] **Step 5: Commit** — `M-c3.0.4: share content triggers Rule 4 cooldown for one turn`.
...
- [ ] **Step 1: Failing test** — `tests/send_url_scheme.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_url_scheme`.
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_url_scheme`.
- [ ] **Step 5: Commit** — `M-c3.1.1: tauri.conf.json carries per-agent muragent-<slug> URL scheme`.
...
- [ ] **Step 1: Failing test** — append to `send_url_scheme.rs`:
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_url_scheme`.
- [ ] **Step 5: Commit** — `M-c3.1.2: muragent-<slug>://share?text=&type= deep-link parser`.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_url_scheme`.
- [ ] **Step 3: Implement** — `mur-agent-gui/src-tauri/src/test_harness.rs` exposes `mock_app(home, slug)` returning a struct with `simulate_open_url` and `captured_payloads`. Internally it wires a `RecordingIngestor` (Vec-backed) instead of `DefaultIngestor`. (`// tauri-2 surface — verify on impl: Tauri 2 mock builder is tauri::test::mock_builder()`)
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_url_scheme`.
- [ ] **Step 5: Commit** — `M-c3.1.3: deep-link → ingestor E2E via Tauri mock builder`.

### Task M-c3.1.4: `agent_export_gui` slug rewrite

**Files:** Modify `mur-core/src/cmd/agent_export_gui.rs`

- [ ] **Step 1: Failing test** — `mur-core/tests/agent_export_gui_url_scheme.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test agent_export_gui_url_scheme`.
- [ ] **Step 3: Implement** — extend `phase_4_rewrite_tauri_conf` to call a new `rewrite_url_scheme(payload_dir, slug)` that JSON-loads, mutates the array, and atomically rewrites. Slug is `slug(&profile.name)` (kebab-case lowered).
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test agent_export_gui_url_scheme`.
- [ ] **Step 5: Commit** — `M-c3.1.4: agent_export_gui injects per-agent slug into urlSchemes`.
...
- [ ] **Step 1: Failing test** — `mur-agent-gui/src-tauri/tests/send_hotkey.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_hotkey`.
- [ ] **Step 3: Implement**:
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_hotkey`.
- [ ] **Step 5: Commit** — `M-c3.2.1: default hotkey combo derives from agent slug`.
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_hotkey`.
- [ ] **Step 5: Commit** — `M-c3.2.2: hotkey reads text/url/image from clipboard`.
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_hotkey`.
- [ ] **Step 5: Commit** — `M-c3.2.3: hotkey collision handling + user override`.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_hotkey`.
- [ ] **Step 3: Implement** — `test_harness::mock_app` registers the shortcut against an in-process `MockShortcutBus`; `trigger_shortcut` invokes the registered callback synchronously. (`// tauri-2 surface — verify on impl: tauri-plugin-global-shortcut may expose a builder for mock dispatch`)
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_hotkey`.
- [ ] **Step 5: Commit** — `M-c3.2.4: hotkey E2E via mock shortcut bus`.
...
- [ ] **Step 1: Failing test** — `mur-agent-gui/src-tauri/tests/send_services.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_services`.
- [ ] **Step 3: Implement** — Cargo gate already added in M-c3.1.1. Stub:
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_services` (skipped on Linux/Windows).
- [ ] **Step 5: Commit** — `M-c3.3.1: services.rs scaffolding + objc2 deps gated to macOS`.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_services`.
- [ ] **Step 3: Implement** — register an `NSObject` subclass via `ClassBuilder::new("MurServicesProvider", NSObject::class())`, add `serviceShare:userData:error:` selector, install via:
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_services`.
- [ ] **Step 5: Commit** — `M-c3.3.2: NSServiceProviderProtocol registers serviceShare: selector`.
...
- [ ] **Step 1: Failing test** — `mur-core/tests/agent_export_gui_nsservices.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test agent_export_gui_nsservices`.
- [ ] **Step 3: Implement** — `Info.plist.template` ships an `NSServices` array with three entries (text, url, image), each with `NSMessage=serviceShare`, `NSPortName={{AGENT_DISPLAY}}`, `NSMenuItem.default = "Send {Selection|Link|Image} to {{AGENT_DISPLAY}}"`. `rewrite_nsservices` is a string template substitution.
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test agent_export_gui_nsservices`.
- [ ] **Step 5: Commit** — `M-c3.3.3: NSServices Info.plist entries injected per-agent`.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_services`.
- [ ] **Step 3: Implement** — `test_harness::mock_app` exposes `invoke_services_selector(pb)` which calls the registered selector body directly (bypasses the NSApplication round-trip).
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_services` (macOS-only, runs no-op on other platforms).
- [ ] **Step 5: Commit** — `M-c3.3.4: services selector E2E on macOS`.
...
- [ ] **Step 1: Failing test** — `mur-agent-gui/src-tauri/tests/send_dock.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_dock`.
- [ ] **Step 3: Implement** — `tauri.conf.json`:
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_dock`.
- [ ] **Step 5: Commit** — `M-c3.4.1: tauri.conf fileAssociations for text/url/image/pdf`.
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_dock`.
- [ ] **Step 5: Commit** — `M-c3.4.2: dock RunEvent::Opened routes through classify_path`.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/src-tauri && cargo test --test send_dock`.
- [ ] **Step 3: Implement** — `test_harness::mock_app::simulate_opened(paths)` constructs a `RunEvent::Opened { urls }` and pumps it through the registered handler.
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/src-tauri && cargo test --test send_dock`.
- [ ] **Step 5: Commit** — `M-c3.4.3: dock E2E for multi-URL Opened event`.
...
- [ ] **Step 1: Failing test** — `mur-agent-gui/ui/src/lib/share.test.ts`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/ui && npm test`.
- [ ] **Step 3: Implement**:
...
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/ui && npm test`.
- [ ] **Step 5: Commit** — `M-c3.5.1: TS share handler with badge + composer insert`.

### Task M-c3.5.2: Visual treatment + "where this came from" accordion

**Files:** Create `mur-agent-gui/ui/src/components/ShareBadge.tsx`

- [ ] **Step 1: Failing test** — `mur-agent-gui/ui/src/components/ShareBadge.test.tsx`:
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/ui && npm test`.
- [ ] **Step 3: Implement** — colored border (Tailwind `border-l-4 border-amber-400 bg-amber-50/40`) + `<details>` element with the channel label and metadata. Channel labels: `url_scheme → "Shared via URL scheme"`, `hotkey → "Shared via hotkey"`, `services → "Shared via Services menu"`, `dock → "Shared by dropping on dock"`.
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/ui && npm test`.
- [ ] **Step 5: Commit** — `M-c3.5.2: ShareBadge with colored border + expandable detail`.
...
- [ ] **Step 2: Verify FAIL** — `cd mur-agent-gui/ui && npm run test:e2e`.
- [ ] **Step 3: Implement** — query-string handler in `App.tsx` invokes `handleShareReceived` for `mockShare=` (test-only). First snapshot run auto-generates the baseline.
- [ ] **Step 4: Verify PASS** — `cd mur-agent-gui/ui && npm run test:e2e`.
- [ ] **Step 5: Commit** — `M-c3.5.3: Playwright snapshot coverage of composer share state`.
...
- [ ] **Step 1: Failing test** — `scripts/e2e/c3-send-from-any-app.sh` exists and is executable.
...
- [ ] **Step 2: Verify FAIL** — `bash scripts/e2e/c3-send-from-any-app.sh` (script not yet executable).
- [ ] **Step 3: Implement** — `chmod +x` + the four `--self-test=*` modes in `mur-agent-gui/src-tauri/src/lib.rs` (mode triggers a programmatic ingest then asserts the resulting telemetry).
- [ ] **Step 4: Verify PASS** — `bash scripts/e2e/c3-send-from-any-app.sh` from a clean home.
- [ ] **Step 5: Commit** — `M-c3.6.1: c3-send-from-any-app.sh exercises all four channels`.

### Task M-c3.6.2: Cookbook

**Files:** Create `docs/cookbook/c3-send-from-any-app.md`

- [ ] **Step 1: Failing test** — `mur-core/tests/verify_c3_cookbook.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test verify_c3_cookbook`.
- [ ] **Step 3: Implement** — write the cookbook covering: setup per-channel, the per-agent slug constraint (each agent registers `muragent-<slug>://` and a slug-suffixed hotkey), the multi-agent escape hatch (override hotkey via `mur agent companion settings <name> --share-hotkey "..."`), and what's not in v1 (unified `mur://` scheme + agent-picker `.appex` Share Extension is v2).
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test verify_c3_cookbook`.
- [ ] **Step 5: Commit** — `M-c3.6.2: c3 cookbook with per-channel walkthroughs`.
...
- [ ] **Step 1: Failing test** — `mur-core/tests/verify_c3_spec_tick.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test verify_c3_spec_tick`.
- [ ] **Step 3: Implement** — append `**Status: shipped 2026-05-04** (PRs cascade-merged: M-c3.0..M-c3.6)` to the §5.5 acceptance footer in the roadmap spec.
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test verify_c3_spec_tick`.
- [ ] **Step 5: Commit** — `M-c3.6.3: §5.5 acceptance footer marked shipped`.
