...
- [ ] **Step 1: Failing test** — append to `mur-common/src/bridge/telegram_config.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-common --lib bridge::telegram_config` (compile error: type missing).

- [ ] **Step 3: Implement** — body of `telegram_config.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-common --lib bridge::telegram_config` (5/5 pass).

- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/ && git commit -m "M-c2.0.1: TelegramConfig + PrivacyMode schema"`

### Task M-c2.0.2: Wire `Platform::Telegram` enum variant (stub arm)

**Files:** Modify `mur-core/src/cmd/agent_companion/connector.rs`.

- [ ] **Step 1: Failing test** — create `mur-core/tests/c2_setup_flow.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test c2_setup_flow` (panics: stderr does not contain expected message).

- [ ] **Step 3: Implement** — in `connector.rs`, extend the `Platform` enum + match:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test c2_setup_flow` (1/1 pass).

- [ ] **Step 5: Commit** — `git add mur-core/ && git commit -m "M-c2.0.2: Platform::Telegram enum stub"`
...
- [ ] **Step 1: Failing test** — append to `mur-core/src/bridge_keychain.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --lib bridge_keychain` (module missing).

- [ ] **Step 3: Implement** — first add to `mur-core/Cargo.toml`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --lib bridge_keychain` (2/2 pass).

- [ ] **Step 5: Commit** — `git add mur-core/ && git commit -m "M-c2.1.1: keychain abstraction + MockKeychain"`

### Task M-c2.1.2: `scaffold_telegram_bridge` 5-step BotFather UX

**Files:** Modify `mur-core/src/cmd/agent_companion/connector.rs`.

- [ ] **Step 1: Failing test** — extend `mur-core/tests/c2_setup_flow.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test c2_setup_flow` (compile error: function missing).
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test c2_setup_flow` (3/3 pass: stub + 2 new).

- [ ] **Step 5: Commit** — `git add mur-core/ && git commit -m "M-c2.1.2: scaffold_telegram_bridge 5-step UX"`

### Task M-c2.1.3: E2E disclosure ack hard-gate

**Files:** Modify `mur-core/src/cmd/agent_companion/connector.rs`.

- [ ] **Step 1: Failing test** — extend `c2_setup_flow.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test c2_setup_flow ack_text_must_match_literal` (function missing).

- [ ] **Step 3: Implement** — append to `connector.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test c2_setup_flow` (4/4 pass).

- [ ] **Step 5: Commit** — `git add mur-core/ && git commit -m "M-c2.1.3: E2E disclosure literal-match gate"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test c2_setup_flow cli_scaffold_via_stdin_script` (CLI flags missing).

- [ ] **Step 3: Implement** — extend `cmd_connector_add` to accept `--bot-token`, `--bot-username`, `--chat-id`, `--ack` (non-interactive path). When `MUR_TELEGRAM_KEYCHAIN_BACKEND=mock`, instantiate `MockKeychain`; otherwise `SystemKeychain`.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test c2_setup_flow` (5/5 pass).

- [ ] **Step 5: Commit** — `git add mur-core/ && git commit -m "M-c2.1.4: CLI scaffold non-interactive flags + integration test"`
...
- [ ] **Step 1: Failing test** — temp test in `mur-agent-runtime/tests/c2_telegram_inbound.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound` (unresolved import).

- [ ] **Step 3: Implement** — append to `mur-agent-runtime/Cargo.toml`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound teloxide_imports_compile`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/Cargo.toml && git commit -m "M-c2.2.1: add teloxide 0.13 dep"`
...
- [ ] **Step 1: Failing test** — replace temp test in `c2_telegram_inbound.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound loop_can_be_constructed` (modules missing).

- [ ] **Step 3: Implement** — `mur-agent-runtime/src/bridge/telegram/mod.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound loop_can_be_constructed`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.2.2: TelegramInboundLoop skeleton + MockBot"`

### Task M-c2.2.3: Inbound loop body — dedupe + privacy + sign + ACK

**Files:** Modify `mur-agent-runtime/src/bridge/telegram/inbound.rs`.

- [ ] **Step 1: Failing test** — extend `c2_telegram_inbound.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound` (compile error: `tick_once` + `InboundDeps` missing).

- [ ] **Step 3: Implement** — flesh out `inbound.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound` (4/4 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.2.3: inbound loop dedupe + privacy + ACK"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound signed_envelope_reaches_user_agent`.

- [ ] **Step 3: Implement** — extend `MockUserAgent` in `mock.rs` with a `Vec<ReceivedReq>` and a `handle()` returning a clone-able `Sender`. In `tick_once`, when a `user_agent` is present, call `sign_payload` over the canonical-JSON of the JsonRpcRequest, build `SignedEnvelope`, send.
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound` (5/5 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.2.4: sign + forward to user-agent via A2A"`
...
- [ ] **Step 1: Failing test** — `c2_telegram_voice.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_voice` (module missing).
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_voice` (1/1 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.3.1: handle_voice_update + whisper stub"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_voice voice_oversize_bails`.

- [ ] **Step 3: Implement** — at top of `handle_voice_update`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_voice` (2/2 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.3.2: voice 30 MB cap"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_voice voice_routed_through_inbound_loop`.

- [ ] **Step 3: Implement** — in `tick_once`, branch on update kind:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_voice` (3/3 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.3.3: voice routed through inbound loop"`
...
- [ ] **Step 2: Verify FAIL** — only if fixture missing/corrupted.

- [ ] **Step 3: Implement** — ensure fixture is committed (binary).

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_voice fixture_ogg_exists_and_nonempty`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/tests/fixtures/voice_hello.ogg mur-agent-runtime/tests/c2_telegram_voice.rs && git commit -m "M-c2.3.4: voice fixture sanity check"`
...
- [ ] **Step 1: Failing test** — `c2_telegram_files.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_files` (modules missing).
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_files` (2/2 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.4.1: document/photo handlers + 20 MB cap"`

### Task M-c2.4.2: Wire into inbound loop with caption

**Files:** Modify `inbound.rs`.

- [ ] **Step 1: Failing test** — append to `c2_telegram_files.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_files document_routed_through_inbound_with_caption`.

- [ ] **Step 3: Implement** — extend `tick_once` document branch:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_files` (3/3 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.4.2: documents/photos in inbound loop with caption + sha256"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_files b0_safety_hook_wraps_pdf_text_on_user_agent`.

- [ ] **Step 3: Implement** — `MockUserAgent::with_b0_hook` activates an in-process `B0SafetyHook` chain that pulls the artifact text from the ledger and wraps it. The bridge does not change; the test asserts the user-agent invariant.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_files` (4/4 pass).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.4.3: B0SafetyHook wraps pdf/image text from telegram"`
...
- [ ] **Step 1: Failing test** — `c2_telegram_outbound.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound list_tools_returns_chat_send_message`.
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound list_tools_returns_chat_send_message`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.5.1: MCP stdio skeleton + tools/list"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound chat_send_message_pushes_to_bot` (already in M-c2.5.1 if implemented; otherwise fails).

- [ ] **Step 3: Implement** — already covered in M-c2.5.1; if test fails due to schema drift, fix the field path.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.5.2: chat.send_message dispatches via teloxide"`
...
- [ ] **Step 1: Failing test** — in `c2_setup_flow.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test c2_setup_flow scaffold_registers_mcp_telegram_chat`.

- [ ] **Step 3: Implement** — extend `write_bridge_profile` to also emit a `profile.yaml` snippet:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test c2_setup_flow`.

- [ ] **Step 5: Commit** — `git add mur-core/ && git commit -m "M-c2.5.3: register telegram_chat in mcp_servers"`
...
- [ ] **Step 2: Verify FAIL** — only if dispatch breaks under sequence.

- [ ] **Step 3: Implement** — confirmed by M-c2.5.2; this test is a regression net.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.5.4: MCP stdio sequence regression test"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound throttle_caps_global_to_thirty_per_second`.

- [ ] **Step 3: Implement** — extend `MockBot` with a `throttled(rate)` constructor that uses an internal `governor` rate-limiter to defer `sent_messages.push`. Track `delivered_within(dur)` from the first send.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound throttle_caps_global_to_thirty_per_second`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.6.1: throttle global 30/s cap"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound per_chat_paces_at_one_per_second`.

- [ ] **Step 3: Implement** — `MockBot::per_chat(rate)` keeps a `HashMap<i64, Instant>` of last-sent and sleeps to enforce the gap.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_outbound per_chat_paces_at_one_per_second`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.6.2: per-chat 1/s pacing"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound bridge_beacon_reports_running_within_5s`.

- [ ] **Step 3: Implement** — `spawn_telegram_bridge_for_test` constructs a `BridgeBeacon` (already shipped in C1 — M-c1.4.3) for the bridge_id `tg` and starts the inbound loop on a background task.

- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test c2_telegram_inbound`.

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/ && git commit -m "M-c2.6.3: BridgeBeacon visible to mur agent doctor for tg bridge"`
...
- [ ] **Step 1: Failing test** — run the script (it does not exist):

```bash
bash scripts/e2e/c2-telegram-bridge.sh
```

- [ ] **Step 2: Verify FAIL** — `bash: scripts/e2e/c2-telegram-bridge.sh: No such file or directory`.

- [ ] **Step 3: Implement** — the script body:
...
- [ ] **Step 4: Verify PASS** — `bash scripts/e2e/c2-telegram-bridge.sh`.

- [ ] **Step 5: Commit** — `git add scripts/e2e/c2-telegram-bridge.sh && git commit -m "M-c2.7.1: c2 telegram bridge e2e gate"`

### Task M-c2.7.2: Cookbook `docs/cookbook/c2-telegram-bridge.md`

**Files:** Create `docs/cookbook/c2-telegram-bridge.md`.

- [ ] **Step 1: Failing test** — `mur verify --file docs/cookbook/c2-telegram-bridge.md` would fail (file missing).

- [ ] **Step 2: Verify FAIL** — file does not exist.

- [ ] **Step 3: Implement** — content covers:
...
- [ ] **Step 4: Verify PASS** — `mur verify --file docs/cookbook/c2-telegram-bridge.md` (no stale claims).

- [ ] **Step 5: Commit** — `git add docs/cookbook/c2-telegram-bridge.md && git commit -m "M-c2.7.2: c2 cookbook"`
...
- [ ] **Step 1: Failing test** — `grep -A1 "## §5.4" docs/superpowers/specs/2026-04-30-mur-agent-harness-roadmap-design.md | grep -i "shipped"` — expected to fail before update.

- [ ] **Step 2: Verify FAIL** — grep returns 1 (no match).

- [ ] **Step 3: Implement** — append to §5.4 acceptance section:
...
- [ ] **Step 4: Verify PASS** — `grep -i "shipped 2026-05-04" docs/superpowers/specs/2026-04-30-mur-agent-harness-roadmap-design.md`.

- [ ] **Step 5: Commit** — `git add docs/superpowers/specs/2026-04-30-mur-agent-harness-roadmap-design.md && git commit -m "M-c2.7.3: §5.4 acceptance footer tick"`
