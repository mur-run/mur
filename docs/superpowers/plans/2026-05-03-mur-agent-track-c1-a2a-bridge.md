...
- [ ] **Step 1: Failing test** — append to a new `mur-common/src/bridge/llm_entitlement.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-common llm_entitlement`. Expect: module not found.

- [ ] **Step 3: Implement** — write the module body above the test:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-common llm_entitlement` (2 tests) and `cargo test -p mur-common --lib agent::tests` (existing AgentProfile tests still parse legacy YAML).

- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/ mur-common/src/agent.rs mur-common/src/lib.rs && git commit -m "M-c1.0.1: LlmEntitlement { mode: allowed|off } + AgentProfile field"`
...
- [ ] **Step 1: Locate LLM builder** — `grep -rn "fn build\|fn new" mur-agent-runtime/src/llm/ | head`. Find the public entry, e.g. `pub fn build_client(profile: &AgentProfile) -> Result<…>`.

- [ ] **Step 2: Failing test** — `mur-agent-runtime/tests/bridge_llm_off_blocks.rs`:
...
- [ ] **Step 3: Verify FAIL** — `cargo test -p mur-agent-runtime --test bridge_llm_off_blocks`.

- [ ] **Step 4: Add guard** — at top of the LLM builder identified in Step 1:
...
- [ ] **Step 5: Verify PASS** — `cargo test -p mur-agent-runtime --test bridge_llm_off_blocks`.

- [ ] **Step 6: Commit** — `git add mur-agent-runtime/src/llm/ mur-agent-runtime/tests/bridge_llm_off_blocks.rs mur-agent-runtime/tests/fixtures/ && git commit -m "M-c1.0.2: supervisor refuses LLM when llm.mode = off"`

### Task M-c1.0.3: One-line CLAUDE.md doc

- [ ] **Step 1: Edit** — In `CLAUDE.md`, in the "Agent Runtime (murmur P0a)" section, append:
...
- [ ] **Step 2: Commit** — `git add CLAUDE.md && git commit -m "M-c1.0.3: doc llm entitlement"`
...
- [ ] **Step 1: Failing test** — write the test alongside types in `routes.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-common bridge::routes`.

- [ ] **Step 3: Implement** — top of `routes.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-common bridge::routes`.

- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/routes.rs mur-common/src/bridge/mod.rs && git commit -m "M-c1.1.1: BridgeRouteConfig schema + serde round-trip"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-common bridge::routes::tests::resolve_falls_back`.

- [ ] **Step 3: Implement** — append to `routes.rs`:
...
- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/routes.rs && git commit -m "M-c1.1.2: BridgeRouteConfig::resolve fallback"`

### Task M-c1.1.3: Precedence — mention > chat_id > default + fanout

**Files:** Modify `mur-common/src/bridge/routes.rs` tests only.

- [ ] **Step 1: Add tests** — append to `mod tests`:
...
- [ ] **Step 2: Verify PASS** — `cargo test -p mur-common bridge::routes`. The resolver from M-c1.1.2 already implements this; if any test fails, fix the resolver until green.

- [ ] **Step 3: Commit** — `git add mur-common/src/bridge/routes.rs && git commit -m "M-c1.1.3: precedence + fanout tests"`
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-common bridge::routes::tests::empty_default_route`.

- [ ] **Step 3: Implement** — append to the `impl BridgeRouteConfig` block:
...
- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/routes.rs && git commit -m "M-c1.1.4: BridgeRouteConfig::validate"`
...
- [ ] **Step 1: Workspace dep** — in root `Cargo.toml` `[workspace.dependencies]`, append `sled = "0.34"`. In `mur-agent-runtime/Cargo.toml` `[dependencies]`, append `sled = { workspace = true }`.

- [ ] **Step 2: Skeleton** — create `mur-agent-runtime/src/bridge/mod.rs`:
...
- [ ] **Step 3: Build** — `cargo build -p mur-agent-runtime`. Warnings about unused fields are fine.

- [ ] **Step 4: Commit** — `git add Cargo.toml Cargo.lock mur-agent-runtime/Cargo.toml mur-agent-runtime/src/bridge/ mur-agent-runtime/src/lib.rs && git commit -m "M-c1.2.1: sled dep + DedupeStore::open skeleton"`

### Task M-c1.2.2: `mark_seen` / `is_seen`

- [ ] **Step 1: Failing test** — `mur-agent-runtime/tests/bridge_dedupe_sled.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test bridge_dedupe_sled`.

- [ ] **Step 3: Implement** — append to `dedupe.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test bridge_dedupe_sled` (3 tests).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/src/bridge/dedupe.rs mur-agent-runtime/tests/bridge_dedupe_sled.rs && git commit -m "M-c1.2.2: mark_seen / is_seen w/ bridge_id namespace"`

### Task M-c1.2.3: TTL eviction

- [ ] **Step 1: Failing test** — append to `bridge_dedupe_sled.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test bridge_dedupe_sled ttl_eviction`.

- [ ] **Step 3: Replace stub + add helper** — replace `sweep_expired` body:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test bridge_dedupe_sled` (4 tests).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/src/bridge/dedupe.rs mur-agent-runtime/tests/bridge_dedupe_sled.rs && git commit -m "M-c1.2.3: TTL sweep evicts entries > 7d"`
...
- [ ] **Step 2: Verify PASS** — `cargo test -p mur-agent-runtime --test bridge_dedupe_sled` (6 tests). Sled flushes on Drop in v0.34, so persistence works without explicit flush.

- [ ] **Step 3: Commit** — `git add mur-agent-runtime/tests/bridge_dedupe_sled.rs && git commit -m "M-c1.2.4: persistence + idempotency"`
...
- [ ] **Step 1: Failing test** — `mur-common/src/bridge/envelope.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-common bridge::envelope`.

- [ ] **Step 3: Implement** — top of `envelope.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-common bridge::envelope`.

- [ ] **Step 5: Commit** — `git add mur-common/Cargo.toml mur-common/src/bridge/ && git commit -m "M-c1.3.1: SignedEnvelope schema"`

### Task M-c1.3.2: `sign_payload` / `verify_envelope_with_pubkey`

- [ ] **Step 1: Failing tests** — append to `envelope.rs` `mod tests`:
...
- [ ] **Step 3: Implement** — append to `envelope.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-common bridge::envelope` (4 tests).

- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/envelope.rs && git commit -m "M-c1.3.2: sign_payload + verify_envelope_with_pubkey"`

### Task M-c1.3.3: `TrustedPeer` + `AgentProfile.trusted_peers`

- [ ] **Step 1: Failing test** — `mur-common/src/bridge/peer.rs`:
...
- [ ] **Step 2: Verify** — `cargo test -p mur-common bridge::peer`.

- [ ] **Step 3: Add to AgentProfile** — in `mur-common/src/agent.rs` `AgentProfile`, before `created_at`:
...
- [ ] **Step 4: Verify back-compat** — `cargo test -p mur-common --lib agent::tests`.

- [ ] **Step 5: Commit** — `git add mur-common/src/bridge/peer.rs mur-common/src/bridge/mod.rs mur-common/src/agent.rs && git commit -m "M-c1.3.3: TrustedPeer + AgentProfile.trusted_peers"`

### Task M-c1.3.4: `verify_inbound_envelope`

- [ ] **Step 1: Failing test** — `mur-agent-runtime/tests/bridge_envelope_signing.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test bridge_envelope_signing`.

- [ ] **Step 3: Implement** — `mur-agent-runtime/src/bridge/verify.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test bridge_envelope_signing` (4 tests).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/src/bridge/verify.rs mur-agent-runtime/src/bridge/mod.rs mur-agent-runtime/tests/bridge_envelope_signing.rs && git commit -m "M-c1.3.4: verify_inbound_envelope rejects untrusted + version-mismatched"`

### Task M-c1.3.5: Module-level transport-independence doc

- [ ] **Step 1: Edit** — In `mur-agent-runtime/src/bridge/mod.rs`, the module doc was added in M-c1.2.1 — append a "Trust model" section to the existing `//!` block:
...
- [ ] **Step 2: Build** — `cargo build -p mur-agent-runtime`.

- [ ] **Step 3: Commit** — `git add mur-agent-runtime/src/bridge/mod.rs && git commit -m "M-c1.3.5: doc transport-independence of envelope verify"`
...
- [ ] **Step 1: Failing test** — `mur-agent-runtime/src/bridge/beacon.rs`:
...
- [ ] **Step 2: Verify** — `cargo test -p mur-agent-runtime bridge::beacon::tests`. PASS.

- [ ] **Step 3: Commit** — `git add mur-agent-runtime/src/bridge/beacon.rs mur-agent-runtime/src/bridge/mod.rs && git commit -m "M-c1.4.1: BridgeBeacon emits telemetry/bridge_alive every 30 s"`

### Task M-c1.4.2: `bridge_status_for_peer` (degraded threshold)

- [ ] **Step 1: Failing test** — `mur-agent-runtime/tests/bridge_beacon_degraded.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-agent-runtime --test bridge_beacon_degraded`.

- [ ] **Step 3: Implement** — append to `beacon.rs`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-agent-runtime --test bridge_beacon_degraded` (3 tests).

- [ ] **Step 5: Commit** — `git add mur-agent-runtime/src/bridge/beacon.rs mur-agent-runtime/tests/bridge_beacon_degraded.rs && git commit -m "M-c1.4.2: bridge_status_for_peer (running/degraded/offline)"`

### Task M-c1.4.3: Supervisor spawns beacon when `llm.mode = off`

- [ ] **Step 1: Locate supervisor** — `grep -n "Acquire running.lock\|Write running.lock\|telemetry_tx" mur-agent-runtime/src/supervisor.rs | head`. Land near `// 8. Write running.lock` (line ~438).

- [ ] **Step 2: Insert spawn** — after `running.lock` is written and `telemetry_tx` is in scope:
...
- [ ] **Step 1: Failing test** — `mur-core/tests/doctor_bridges_section.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test doctor_bridges_section`.

- [ ] **Step 3: Implement** — append to `mur-core/src/cmd/doctor.rs` (and ensure `pub` so the integration test can import):
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test doctor_bridges_section`.

- [ ] **Step 5: Commit** — `git add mur-core/src/cmd/doctor.rs mur-core/tests/doctor_bridges_section.rs mur-core/tests/fixtures/ && git commit -m "M-c1.4.4: mur agent doctor surfaces bridges: section"`
...
- [ ] **Step 1: Failing test** — `mur-agent-runtime/src/bridge/ack.rs`:
...
- [ ] **Step 2: Verify PASS** — `cargo test -p mur-agent-runtime bridge::ack::tests`.

- [ ] **Step 3: Commit** — `git add mur-agent-runtime/src/bridge/ack.rs mur-agent-runtime/src/bridge/mod.rs && git commit -m "M-c1.5.1: AckTracker — confirm/reject + invariant doc"`

### Task M-c1.5.2: 5xx-then-2xx integration

- [ ] **Step 1: Test** — `mur-agent-runtime/tests/bridge_ack_tracker.rs`:
...
- [ ] **Step 2: Verify** — `cargo test -p mur-agent-runtime --test bridge_ack_tracker`. PASS.

- [ ] **Step 3: Commit** — `git add mur-agent-runtime/tests/bridge_ack_tracker.rs && git commit -m "M-c1.5.2: AckTracker — 5xx recovery + pinned-on-failures"`
...
- [ ] **Step 1: Re-read** — `head -20 mur-agent-runtime/src/bridge/ack.rs`. Confirm the doc lines beginning with `//!` describe (a) "advance iff 2xx" and (b) the persistence requirement.

- [ ] **Step 2: Build** — `cargo doc -p mur-agent-runtime --no-deps`. PASS without warnings on the bridge module.

- [ ] **Step 3: Commit (if any tweaks)** — `git diff --quiet || git commit -am "M-c1.5.3: confirm AckTracker doc states the invariant"` (no-op if no changes).
...
- [ ] **Step 1: Locate enum** — `grep -n "enum CompanionCmd" mur-core/src/cmd/agent_companion.rs`.

- [ ] **Step 2: Extend enum** — in `agent_companion.rs`, append a variant:
...
- [ ] **Step 3: Stub** — create `mur-core/src/cmd/agent_companion/connector.rs`:
...
- [ ] **Step 4: Build** — `cargo build -p mur-core`. PASS.

- [ ] **Step 5: Commit** — `git add mur-core/src/cmd/agent_companion.rs mur-core/src/cmd/agent_companion/connector.rs mur-core/src/cmd/agent_companion/mod.rs && git commit -m "M-c1.6.1: wire 'mur agent companion connector add' subcommand"`

### Task M-c1.6.2: Stub bridge scaffold

- [ ] **Step 1: Failing test** — `mur-core/tests/connector_add_stub.rs`:
...
- [ ] **Step 2: Verify FAIL** — `cargo test -p mur-core --test connector_add_stub`.

- [ ] **Step 3: Implement scaffold** — replace the stub `scaffold_stub_bridge`:
...
- [ ] **Step 4: Verify PASS** — `cargo test -p mur-core --test connector_add_stub`.

- [ ] **Step 5: Commit** — `git add mur-core/src/cmd/agent_companion/connector.rs mur-core/tests/connector_add_stub.rs && git commit -m "M-c1.6.2: stub bridge scaffold writes profile + routes + identity"`

### Task M-c1.6.3: Reject duplicate add + non-stub platform

- [ ] **Step 1: Tests** — append to `mur-core/tests/connector_add_stub.rs`:
...
- [ ] **Step 2: Verify** — `cargo test -p mur-core --test connector_add_stub` (3 tests). PASS.

- [ ] **Step 3: Commit** — `git add mur-core/tests/connector_add_stub.rs && git commit -m "M-c1.6.3: reject non-stub platform + dup-add"`
...
- [ ] **Step 1: Create** — `scripts/e2e/c1-bridge-roundtrip.sh`:
...
- [ ] **Step 2: chmod** — `chmod +x scripts/e2e/c1-bridge-roundtrip.sh`.

- [ ] **Step 3: Commit** — `git add scripts/e2e/c1-bridge-roundtrip.sh && git commit -m "M-c1.7.1: E2E shell harness"`

### Task M-c1.7.2: Round-trip integration test

- [ ] **Step 1: Test** — `mur-agent-runtime/tests/bridge_roundtrip.rs`:
...
- [ ] **Step 2: Verify** — `cargo test -p mur-agent-runtime --test bridge_roundtrip` (3 tests). PASS.

- [ ] **Step 3: Commit** — `git add mur-agent-runtime/tests/bridge_roundtrip.rs && git commit -m "M-c1.7.2: stub-bridge full-loop integration test"`

### Task M-c1.7.3: Cookbook

- [ ] **Step 1: Create** — `docs/cookbook/c1-a2a-bridge.md`:
...
- [ ] **Step 2: Commit** — `git add docs/cookbook/c1-a2a-bridge.md && git commit -m "M-c1.7.3: Track C1 cookbook"`

### Task M-c1.7.4: Spec tick

- [ ] **Step 1: Locate footer** — `grep -n "Quiet hours / proactive policy" docs/superpowers/specs/2026-04-30-mur-agent-harness-roadmap-design.md` (line 542).

- [ ] **Step 2: Append** — after line 542, insert:
...
- [ ] **Step 3: Run E2E** — `./scripts/e2e/c1-bridge-roundtrip.sh`. Expect `✅ Track C1 bridge round-trip E2E passed`.
...
- [ ] **Step 4: Commit** — `git add docs/superpowers/specs/2026-04-30-mur-agent-harness-roadmap-design.md && git commit -m "M-c1.7.4: tick §5.1 / §5.2 / §5.3 acceptance footer"`
