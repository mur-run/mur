...
- [ ] **Step 2: Create `mur-common/src/bridge/slack_config.rs`**
...
- [ ] **Step 3: Write the serde tests (inline)**
...
- [ ] **Step 4: Wire into `mur-common/src/bridge/mod.rs`**
...
- [ ] **Step 2: Add dep to `mur-agent-runtime/Cargo.toml`**
...
- [ ] **Step 1: Create `mur-agent-runtime/src/bridge/slack/mod.rs`**
...
- [ ] **Step 2: Wire into `mur-agent-runtime/src/bridge/mod.rs`**
...
- [ ] **Step 3: Create `mur-agent-runtime/src/bridge/slack/socket.rs`**
...
- [ ] **Step 2: Create `mur-agent-runtime/src/bridge/slack/inbound.rs`** (skeleton)
...
- [ ] **Step 3: Create `mur-agent-runtime/src/bridge/slack/mock.rs`**
...
- [ ] **Step 4: Write construction smoke test**
...
- [ ] **Step 2: Write the failing tests** (append to `c7_slack_inbound.rs`)
...
- [ ] **Step 4: Add `TickResult` + `tick_once` to `inbound.rs`**
...
- [ ] **Step 2: Append tests to `c7_slack_inbound.rs`**
...
- [ ] **Step 4: Implement tick_once phase 2 (signing + A2A + ack)**
...
- [ ] **Step 6: Run full runtime test suite (regression check)**
...
- [ ] **Step 2: Create `mur-agent-runtime/src/bridge/slack/reply.rs`**
...
- [ ] **Step 3: Write rate-limit test** (append to `c7_slack_inbound.rs`)
...
- [ ] **Step 2: Read existing connector.rs** to understand the full `add()` function signature and `scaffold_stub_bridge` API.
...
- [ ] **Step 3: Add `"slack"` arm and `run_slack_setup`**
...
- [ ] **Step 4: Check the `Keychain` trait's `set` method name**
...
- [ ] **Step 2: Create `scripts/e2e/c7-slack-bridge.sh`**
...
- [ ] **Step 3: Add C7 stanza to `scripts/e2e/run-all.sh`**
...
- [ ] **Step 1: Create `docs/cookbook/c7-slack-bridge.md`**
...
- [ ] **Step 1: Update spec footer** in `docs/superpowers/specs/2026-05-09-mur-agent-c7-slack-bridge-design.md`
...
- [ ] **Step 2: Retarget all PRs to main, then cascade-merge**
