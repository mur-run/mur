...
- [x] **Step 1.1: Read the current hook entry push block**
...
- [x] **Step 1.2: Replace the hook entry push with event-aware async flags**
...
- [x] **Step 1.3: Write a unit test for the generated settings shape**
...
- [x] **Step 2.1: Create `mur-core/src/cmd/init_daemon.rs`**
...
- [x] **Step 2.2: Register the module in `mur-core/src/cmd/mod.rs`**
...
- [x] **Step 2.3: Call `install_daemon_service()` from `mur-core/src/cmd/init.rs`**
...
- [x] **Step 2.4: Run clippy to catch cfg/platform issues**
...
- [x] **Step 3.1: Add `is_daemon_healthy()` to `mur-core/src/daemon.rs`**
...
- [x] **Step 3.2: Write a unit test for `is_daemon_healthy()`**
...
- [x] **Step 3.4: Add health-check + respawn call in `cmd_hook_prompt`**
...
- [x] **Step 3.5: Confirm no regression in hook tests**
...
- [x] **Step 4.1: Create `mur-core/tests/gate_golden.rs`**
...
- [x] **Step 4.2: Export `Tier` from `mur-core/src/retrieve/gate.rs` for integration tests**
...
- [x] **Step 4.3: Run the golden set test**
...
- [x] **Step 5.1: Add `duration_ms` to `NormalizedEvent`**
...
- [x] **Step 5.2: Record latency in `cmd_hook_prompt` and `cmd_hook_tool`**
...
- [x] **Step 5.3: Add percentile computation to `mur-core/src/inject/stats.rs`**
...
- [x] **Step 5.4: Update `format_stats` to display latency**
...
- [x] **Step 5.5: Write a test for percentile computation**
...
- [x] **Step 5.6: Run all stats + hook tests**
...
- [x] **Open a PR for each task** (5 PRs total, one per task). Each task is independent and can merge in any order. Suggested PR titles:
