...
- [x] **Step 1: Confirm worktree/branch state**
...
- [x] **Step 1: Write a failing test asserting the sources table can be opened/created**
...
- [x] **Step 3: Add the schema and helper methods**
...
- [x] **Step 3: Replace the `upsert` stub with a real implementation**
...
- [x] **Step 1: Replace the `search` trait-method stub**
...
- [x] **Step 2: Run the roundtrip test**
...
- [x] **Step 3: Also verify conformance smoke still passes**
...
- [x] **Step 3: Replace both delete method bodies**
...
- [x] **Step 1: Remove the two `#[ignore = "..."]` lines**
...
- [x] **Step 2: Before the conformance tests run, the LanceDbStore factory used by the macro needs to call `ensure_sources_table` — otherwise search returns empty and upsert fails silently.**
...
- [x] **Step 3: Run conformance suite**
...
- [x] **Step 4: Full workspace regression**
...
- [x] **Step 1: Add pulldown-cmark dep**
...
- [x] **Step 2: Create `sources/chunker/mod.rs`**
...
- [x] **Step 3: Create `sources/chunker/markdown.rs`**
...
- [x] **Step 1: Create `sources/adapters/mod.rs`**
...
- [x] **Step 2: Declare in sources module**
...
- [x] **Step 3: Create `obsidian.rs` with struct, constructor, list_documents (TDD — tests first)**
...
- [x] **Step 4: Add helper functions at the bottom of `obsidian.rs` (outside any impl / mod):**
...
- [x] **Step 5: Ensure `urlencoding` dep exists or add it**
...
- [x] **Step 1: Create `sources/sync.rs` with tests**
...
- [x] **Step 1: Replace the `source_cmd.rs` file with a working dispatcher for three verbs**
...
- [x] **Step 2: Add the `add_obsidian` function at the bottom of the same file**
...
- [x] **Step 3: Add `list` and `status` functions**
...
- [x] **Step 4: Update imports at the top of `source_cmd.rs`**
...
- [x] **Step 5: Add `rand` dep if not present**
...
- [x] **Step 7: Smoke test the three commands against a temp vault**
...
- [x] **Step 1: Append the three handler functions**
...
- [x] **Step 1: Append the two handler functions**
...
- [x] **Step 1: Add `Search` variant to `SourceCommand` enum**
...
- [x] **Step 3: Implement the `search` handler**
...
- [x] **Step 4: The `Hit` struct has no `Serialize` derive — serialize manually via `json!{}` (done above)**

Verify the `use crate::store::vector::...` covers `SearchFilter` and that `Hit` is reachable (it is, via `search` return type).

- [x] **Step 5: Compile + smoke test**
...
- [x] **Step 1: Decide on embedding for E2E**
...
- [x] **Step 2: Write `mur-core/tests/obsidian_e2e.rs`**
...
- [x] **Step 5: Full CLI smoke on real machine**
...
- [x] LanceDB sources table created on demand (`ensure_sources_table`).
- [x] `LanceDbStore::upsert`, `search`, `list_external_ids`, `delete_by_external_ids`, `delete_by_source`, `count` are real — not stubs. `rebuild_index` stays stub (P1.3).
- [x] Conformance tests `conformance_upsert_and_search` + `conformance_delete_by_source_clears` pass (no longer `#[ignore]`).
- [x] `sources::chunker::markdown::chunk_markdown` emits heading-aware chunks; paragraph-boundary split on oversized.
- [x] `ObsidianAdapter` implements `KnowledgeSource` end-to-end: `list_documents` (walkdir + exclude + cursor), `fetch` (read file + strip frontmatter + tags), `chunk` (delegate to markdown chunker).
- [x] `sources::sync::sync_source` performs list → fetch → chunk → embed → upsert, full-mode deletion diff, per-doc error isolation, persists cursor + stats to yaml.
- [x] CLI verbs working for real: `source add obsidian`, `source list`, `source status`, `source sync [id] [--full]`, `source remove [id] [--keep-index]`, `source test`, `source weight`, `source enable`, `source disable`, `source search`.
- [x] CLI verbs still stubbed: `add notion/joplin`, `sync --watch`, `reindex`, `install-schedule` (all return clear "arrives in P1.x" errors).
- [x] E2E integration test covers list → fetch → chunk → upsert → search → delete flow.
- [x] Clippy clean, fmt clean, feature matrix green.
- [x] CLAUDE.md updated.
