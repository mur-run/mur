...
- [x] **Step 1: Create `notion_blocks.rs`**
...
fn render_block(block: &Value, depth: usize, out: &mut String) {
    ...
    match kind {
        ...
        "to_do" => {
            ...
            out.push_str(&indent);
            out.push_str(if checked { "- [x] " } else { "- [x] " });
            push_rich_text(detail, out);
            out.push('\n');
        }
        "quote" => {
...
mod tests {
    ...
    fn to_do_unchecked_renders_brackets() {
        ...
        assert!(blocks_to_markdown(&blocks).contains("- [x] task"));
    }
...
- [x] **Step 1: Add OAuth helper to `notion.rs`**
...
- [x] **Step 2: Wire into `cmd/source_cmd.rs` `add_notion` handler**
...
- [x] **Step 3: Update sync handler to construct NotionAdapter**
...
- [x] **Step 1: Extend `JoplinMode` enum + adapter**
...
- [x] **Step 2: Wire `add_joplin` in source_cmd.rs**
...
- [x] **Step 3: Wire `--watch` in `source_cmd.rs`**
...
- [x] **Step 2: Add the source-fetch helper inside hook.rs**
...
- [x] **Step 1: Write user-facing setup notes**
...
- [x] Notion block-to-markdown chunker with 6 unit tests
- [x] NotionAdapter (PAT + OAuth/PKCE) implements `KnowledgeSource`
- [x] JoplinAdapter (Local SQLite + Server) implements `KnowledgeSource`
- [x] `mur source add notion [--token <pat>] [--workspace <id>]` works
- [x] `mur source add joplin --db <path>` and `--server <url> --token <pat>` work
- [x] `mur source sync --watch` foreground daemon (fsevents + poll)
- [x] `mur source install-schedule` writes launchd plist (macOS) / systemd unit (Linux)
- [x] `inject::format_notes_section` is wired into the inject entry (or follow-up plan filed)
- [x] `docs/source-adapters.md` published
- [x] Tests + clippy + fmt + feature matrix all green
- [x] CLAUDE.md updated
