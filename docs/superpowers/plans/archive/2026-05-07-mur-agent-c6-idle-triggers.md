...
- [x] **Step 3: Add the `IdleTrigger` struct and field**
...
- [x] **Step 3: Add the activity timestamp**
...
- [x] **Step 5: Add the IdleScheduler runtime task**
...
- [x] **Step 3: Add `with_tick_interval` to IdleScheduler**
...
- [x] **Step 6: Verify supervisor still compiles**
...
- [x] **Step 3: Implement the CLI helpers**
...
- [x] **Step 1: Add `Idle` variants to `AgentScheduleAction`**
...
- [x] **Step 2: Run full test + lint suite**
...
- [x] Manual smoke: add trigger to a stub agent, confirm fire after threshold via `~/.mur/agents/<n>/telemetry/<date>.jsonl`
- [x] Cookbook walkthrough at `docs/cookbook/c6-idle-triggers.md`
...
- [x] Every task references **exact file paths** from this plan.
- [x] No `TODO` / `TBD` / "implement later" placeholders.
- [x] Type names match across tasks: `IdleTrigger`, `IdleScheduler`, `should_fire`, `cmd_idle_add/list/remove`, `read_idle_triggers`, `last_activity_at`, `with_tick_interval`.
- [x] Spec coverage: schema (Task 1) ✅, runner activity (Task 2) ✅, scheduler primitive (Task 3) ✅, supervisor wiring (Task 4) ✅, CLI (Task 5) ✅, dispatch (Task 6) ✅, cookbook (Task 7) ✅, version (Task 8) ✅.
- [x] Tests use the same `MurHomeGuard` + `ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner())` pattern as existing C4 tests.
- [x] `cargo fmt` is run before commit (CI requires strict formatter pass; multi-line `unsafe { set_var(...); }` blocks are mandatory).
