...
- [x] **Step 1: Confirm you're on the right branch and tree is clean except for plan/spec**
...
- [x] **Step 2: Confirm existing test suite is green before touching anything**
...
- [x] **Step 2: Add `sources` feature flag**
...
- [x] **Step 1: Write the failing test for `StorageConfig` defaults**
...
- [x] **Step 3: Implement `StorageConfig` + `SourcesGlobalConfig`**
...
- [x] **Step 4: Wire the new fields into `Config`**
...
- [x] **Step 6: Run full workspace tests to confirm no regression**
...
- [x] **Step 1: Create the new module file with trait + types**
...
- [x] **Step 2: Expose it from `store/mod.rs`**
...
- [x] **Step 3: Create placeholder `store/vector/lancedb.rs` and `factory.rs` to satisfy the module tree**
...
- [x] **Step 4: Verify workspace still compiles — existing callers of `crate::store::lancedb` will now fail. That's expected and fixed in Task 4.**
...
- [x] **Step 1: Copy the entire existing `store/lancedb.rs` content to `store/vector/lancedb.rs`**
...
- [x] **Step 2: Rename the struct in the new file from `VectorStore` to `LanceDbStore`**
...
- [x] **Step 3: Add `VectorStore` trait implementation block for `LanceDbStore`**
...
- [x] **Step 4: Update `store/vector/mod.rs` to re-export the renamed struct**
...
- [x] **Step 5: Update every caller of `crate::store::lancedb::VectorStore`**
...
- [x] **Step 6: Delete the old `mur-core/src/store/lancedb.rs` file**
...
- [x] **Step 8: Run the full test suite, including the moved LanceDB tests**
...
- [x] **Step 1: Create the conformance suite**
...
- [x] **Step 2: Wire the LanceDbStore through the macro**
...
- [x] **Step 2: Add `uuid` dependency if not present**
...
- [x] **Step 3: Run tests (expected to fail — module doesn't exist yet)**
...
- [x] **Step 4: Declare the `sources` module**
...
- [x] **Step 1: Create kind.rs with tests**
...
- [x] **Step 2: Declare it in the sources module**
...
- [x] **Step 1: Create credentials.rs**
...
- [x] **Step 1: Create instance.rs with tests**
...
- [x] **Step 1: Rewrite `sources/mod.rs` to host the trait**
...
- [x] **Step 1: Create the stub subcommand module**
...
- [x] **Step 2: Register the module conditionally**
...
- [x] **Step 3: Wire into the top-level CLI in `main.rs`**
...
- [x] **Step 4: Verify the CLI help reflects the new subcommand**
...
- [x] **Step 5: Verify stub execution**
...
- [x] **Step 6: Verify feature-off build hides the subcommand**
...
- [x] **Step 1: Add a line in the Architecture section**
...
- [x] **Step 1: Full workspace test run**
...
- [x] **Step 4: Confirm feature flag off still compiles**
...
- [x] **Step 5: Confirm binary starts and existing commands still work**
...
- [x] **Step 6: Sanity check — the `~/.mur/sources/` directory is NOT created by existing commands**
...
- [x] **Step 7: Close-out commit if anything drifted**
...
- [x] `store::vector::VectorStore` trait defined, with LanceDbStore implementing it (stubs are allowed — P1.2 fills them in).
- [x] `store::vector::factory::get_vector_store(&Config, &Path)` selects backend from config.
- [x] `sources::{types, kind, credentials, instance}` modules compiled with tests.
- [x] `sources::KnowledgeSource` trait defined (no adapter impls yet).
- [x] `Config::storage` + `Config::sources_global` present with zero-action defaults.
- [x] `mur source --help` surfaces the subcommand tree behind `sources` feature flag; verbs return informative "not yet implemented" errors.
- [x] All existing tests still pass; +23 new tests added.
- [x] `CLAUDE.md` architecture section mentions the sources pipeline foundation.
- [x] No `~/.mur/sources/` directory auto-created by any existing code path.
