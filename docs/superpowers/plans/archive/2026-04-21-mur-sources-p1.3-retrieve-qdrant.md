...
- [x] **Step 1: Confirm worktree + HEAD**
...
- [x] **Step 1: Define the wrapper API (TDD)**
...
- [x] **Step 1: Update `sync_source` signature**
...
- [x] **Step 2: Update `cmd/source_cmd.rs` handlers that call `sync_source`**
...
- [x] **Step 2: Implement `score_sources`**
...
- [x] **Step 1: Inspect current retrieve entry**
...
- [x] **Step 1: Locate the existing Search handler**
...
- [x] **Step 2: Add flags to the `Search` variant**
...
- [x] **Step 3: Implement the unified search dispatcher**
...
- [x] **Step 4: Docker compose sample**
...
- [x] **Step 5: Register QdrantStore in conformance macro (opt-in)**
...
- [x] **Step 6: Compile + test (without Qdrant running)**
...
- [x] **Step 1: Replace the reindex stub**
...
- [x] **Step 1: Inspect existing formatter**
...
- [x] Tantivy BM25 index wrapper + sync/remove integration
- [x] `score_sources()` function with source_weight + freshness + length_norm
- [x] `retrieve_unified()` merges vec+BM25 and applies source scoring
- [x] `mur search` has `--source`, `--type`, `--only-sources`, `--only-patterns`, `--json` flags; prints Patterns + Notes sections
- [x] `QdrantStore` implements `VectorStore` trait; factory routes `storage.vector_backend = "qdrant"` to it
- [x] `docker/qdrant-compose.yml` provided for local dev
- [x] Opt-in `tests/qdrant_smoke.rs` gated behind `QDRANT_URL`
- [x] `mur source reindex [--vector-backend <B>]` real; clears both vector store + tantivy, re-runs full sync
- [x] `inject::format_notes_section` produces the Notes section; integrated into the top-level inject entry
- [x] E2E test `retrieve_unified.rs` verifies cross-backend ranking consistency
- [x] Clippy + fmt clean; feature matrix green
- [x] CLAUDE.md updated
