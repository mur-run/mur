...
| P1 | **Landed leaner than planned:** recon showed notes already ARE skills (`Category::Note` + lifecycle stats + `Retrievable` + 4-layer visibility scope with pre-retrieval filtering) — W3b+ was mostly done under a different name. The real delta: `kind: rule\|fact` (tag-encoded, `note_kind()` single reader), per-kind config-driven half-life curves (`note_{rule,fact}_half_life_factor`, sweep + retrieval decay), and the reserved note slot (`retrieval.reserved_note_slots`, default 1) | small-medium |
