...
- **Pure functions (Vitest):** `parseDetailRoute` (agent / fleet / `+` and percent decoding / unknown kind / empty name); `isOpenInWindowShortcut` (meta, ctrl, rejects alt/shift/plain Enter); palette items include `action:openInWindow` only with a selection (via the existing `rankPalette` test style); `SourceList` markup unchanged when `onOpen` is absent (`renderToStaticMarkup`, as its existing test).
