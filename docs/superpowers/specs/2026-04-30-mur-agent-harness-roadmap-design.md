...
- v1 ship status (2026-05-05):
  ...
  - **Rule 6: shipped (M9.1–M9.4)** — `McpServerEntry` pin schema (binary_sha256 + description_hash + publisher + installed_at) + `mur agent mcp add` install-time hashing + `B0SafetyHook::on_startup` re-verify + `mur agent mcp inspect`/`pin` recovery verbs. Description-hash live probe deferred to M9.3.5 (lazy check at first MCP call). Cookbook: `docs/cookbook/b0-mcp-install-verify.md`. Acceptance: `bash scripts/e2e/b0-m9-mcp-install-verifier.sh`.
