...
- **Eval harness shipped 2026-05-06 (M11.0–M11.5 + Phase A)** — `scripts/eval/{agentdojo,harmbench}/` Python runners + `mur agent eval report` Rust aggregator + `.github/workflows/eval.yml` (stub-LLM PR gate + real-LLM weekly cron + per-tag) + `Makefile` (`make eval-stub` / `eval-release`). Both 50-case subsets committed at `scripts/eval/{agentdojo,harmbench}/case_selection.json` with shared seed 1202914782. Cookbook: `docs/cookbook/b0-eval.md`. Audit history: `eval-results/<tag>.jsonl`.
- **M11.6 v2.7.0 HarmBench baseline shipped 2026-05-06** — run `01KQYJDTKZWBBX239MYHRRT320`, `claude-sonnet-4-6` @ T=0, 50 cases × 1 turn = 1331 input + 11019 output tokens. **Aggregate: 41/50 = 82% PASS** (spec gate ≥ 50%). Per-category: chemical_biological 7/7, copyright 8/13, cybercrime_intrusion 4/8, harassment_bullying 3/3, harmful 3/3, illegal 8/8, misinformation_disinformation 8/8. Failure pattern: 5 copyright passages + 2 SQL-injection templates + 1 zero-day market explainer + 1 heuristic false-negative on a "No. This isn't…" refusal. Grader: `heuristic_v1` (refusal-pattern matching per HarmBench paper §3.2). Each JSONL record stores raw `response_text` so a future Phase B re-grade against `cais/HarmBench-Llama-2-13b-cls` produces a `classifier_v1` revision **with no API re-spend**. AgentDojo-50 still pending Phase B (upstream benchmark loop wire-up). Artifacts: `eval-results/v2.7.0.{jsonl,md}`.
